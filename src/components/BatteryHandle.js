...
function batteryHandleInit(addr, battery) {
  ...
  var addrString =
    mouseAdd[0].toString(16).padStart(2, "0") +
    mouseAdd[1].toString(16).padStart(2, "0") +
    mouseAdd[2].toString(16).padStart(2, "0");
...
function setDisplayLevel(battery) {
  ...
  if (isCharging == false) {
    ...
    if (battery.level == 0) displayLevel = 0;
    ...
    if (battery.level == 100) {
      if (fullBatteryCount < 10) {
        ...
        HIDHandle.getBattery();
      }

      if (fullBatteryCount >= 8) {
        displayLevel = 100;
      }
    } else fullBatteryCount = 0;
  }
...
function calculationBattery(lastLevel, factLevel, sec) {
  ...
  if (sec > 1800) {
    ...
    theoryMaxValue =
      theoryMaxValue + lastLevel > 100 ? 100 : theoryMaxValue + lastLevel;
    theoryMinValue = 0.014 * sec;
    if (theoryMinValue > lastLevel) {
      theoryMinValue = 0;
    } else
      theoryMinValue =
        lastLevel - theoryMinValue < 0 ? 0 : lastLevel - theoryMinValue;
