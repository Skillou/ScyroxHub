...
async function requestDevice(filters) {
  ...
  for (let temp of devices) {
    for (let i = 0; i < temp.collections.length; i++) {
      if (
        temp.collections[i].inputReports.length === 1 &&
      ...
      if (receivedData[1] == 0) {
        switch (command) {
          ...
          case Command.BatteryLevel:
            ...
            deviceInfo.battery.voltage =
              (receivedData[7] << 8) + receivedData[8];
            if (deviceInfo.batteryOptimize == false) {
              if (deviceInfo.batteryOptimizeInit == false)
                BatteryHandle.batteryHandleInit(
                  deviceInfo.addr,
                  deviceInfo.battery
                );
          ...
          case Command.ReadFlashData:
            ...
            if (
              (addr == MouseEepromAddr.ReportRate && len == 2) ||
...
async function setArrayEeprom(address, value) {
  ...
  cnt =
    cnt > 0 ? Math.floor(value.length / 10) + 1 : Math.floor(value.length / 10);
...
async function getOnlineInterval() {
  ...
  if (flag) {
    ...
    } else mouseCfg.supportLongDistance = false;
...
function bufferToColor(buffer, index) {
  var color =
    "#" +
    buffer[index].toString(16).padStart(2, "0") +
    buffer[index + 1].toString(16).padStart(2, "0") +
    buffer[index + 2].toString(16).padStart(2, "0");
  return color;
}
...
async function updateDeviceInfo() {
  ...
  mouseCfg.sensor.sensor20K = flashData[0x08] == 1;
...
async function Get_Mouse_FunctionKeys() {
  if ((mouseCfg.connectState = DeviceConectState.Connecting)) {
    ...
    for (var i = 0; i < 16; i++) {
      ...
      var value = [
        flashData[addr].toString(16),
        "0x" + tmp.toString(16).padStart(4, "0"),
      ];
        ...
        if (contexts.length == 1) {
          if (contexts[0].type == 2) {
            ...
            var context = {
              ...
              value:
                "0x" +
                contexts[0].value.toString(16).padStart(4, "0").toUpperCase(),
            };
...
function updateShortCutKey(index) {
  ...
  for (var i = 0; i < count / 2; i++) {
    ...
    var value =
      (flashData[addr + i * 0x03 + 3] << 8) + flashData[addr + i * 0x03 + 2];
    var context = {
      type: type,
      value: value,
    };
...
function updateMacro(index) {
  ...
  var context;
  if (nameLen <= 30 && nameLen > 0 && contextLen <= 70 && contextLen >= 2) {
    var names = [];
...
async function setMaxDPI(value) {
  ...
  if (flag == true) await setByteEeprom(MouseEepromAddr.MaxDPI, value);
}

async function setCurrentDPI(value) {
  var flag = await getOnline();

  if (flag == true) await setByteEeprom(MouseEepromAddr.CurrentDPI, value);
}
...
async function setDPILightSpeed(value) {
  ...
  if (flag == true) await setByteEeprom(0x50, value);
}

async function setDPILightOff() {
  var flag = await getOnline();

  if (flag == true) await setByteEeprom(0x52, 0);
}

async function setRGBColor(value) {
  var flag = await getOnline();

  if (flag == true) await setArrayEeprom(0x54, value);
}

async function setRGBEffect(value) {
  var flag = await getOnline();

  if (flag == true) await setByteEeprom(0x58, value);
}

async function setRGBSpeed(value) {
  var flag = await getOnline();

  if (flag == true) await setByteEeprom(0x5a, value);
}

async function setRGBBri(value) {
  var flag = await getOnline();

  if (flag == true) await setByteEeprom(0x5c, value);
}

async function setLightPowerSave(value) {
  var flag = await getOnline();

  if (flag == true) await setByteEeprom(0x5e, value);
}
...
async function setLOD(value) {
  ...
  if (flag == true) await setByteEeprom(0x0a, value);
}

async function setDebounceTime(value) {
  var flag = await getOnline();

  if (flag == true) await setByteEeprom(0xa9, value);
}

async function setMotionSync(value) {
  var flag = await getOnline();

  if (flag == true) await setByteEeprom(0xab, value);
}

async function setMoveLightOffTime(value) {
  var flag = await getOnline();

  if (flag == true) await setByteEeprom(0xad, value);
}

async function setFixLine(value) {
  var flag = await getOnline();

  if (flag == true) await setByteEeprom(0xaf, value);
}

async function setRipple(value) {
  var flag = await getOnline();

  if (flag == true) await setByteEeprom(0xb1, value);
}

async function setSensor20K(value) {
  var flag = await getOnline();

  if (flag == true) await setByteEeprom(0x08, value);
}

async function setMoveLightOffState(value) {
  var flag = await getOnline();

  if (flag == true) await setByteEeprom(0xb3, value);
}

async function setPerformanceState(value) {
  var flag = await getOnline();

  if (flag == true) await setByteEeprom(0xb5, value);
}
...
async function setSensorMode(value) {
  ...
  if (flag == true) await setByteEeprom(MouseEepromAddr.SensorMode, value);
}
...
async function setShortCutKey(index, shortCut) {
  ...
  if (flag == true) {
    ...
    for (var i = 0; i < cnt; i++) {
      ...
      shortCutKey.contexts.push(context);
    }

    mouseCfg.shortCutKey[index] = shortCutKey;
  }
}
...
async function getShortCutKey(index) {
  ...
  if (count > 2) {
    ...
    for (var i = 0; i < cnt; i++) {
      ...
      await getEepromBuffer(
        MouseEepromAddr.ShortcutKey + index * 0x20 + (i + 1) * 10,
        len
      );
    }
  }
...
async function restoreMacro(index) {
  ...
  if (flag == true) await setArrayEeprom(addr, value);
}
...
async function setRFTXTime(value) {
  ...
  if (flag == true) await setByteEeprom(0xbb, value);
}
